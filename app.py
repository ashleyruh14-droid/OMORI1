...
# ---------------------------------------------------
st.set_page_config(
    page_title="Contrôle de poids OMORI 1",
    page_icon="🧪",
    layout="wide"
)
...
def get_poids_min(poids_produit):
    """Retourne le poids minimum toléré pour OMORI 1."""
    if not poids_produit:
        return None
    p = int(round(poids_produit))
    return POIDS_MIN_OMORI1.get(p)
...
def write_log(
    ...
    Si le fichier n'existe pas (par ex. tu l'as supprimé pour repartir à zéro),
    il est recréé avec un en-tête propre.
    """
    ...
    try:
        ...
        st.error(
            ...
            "Le fichier d'historique (historique_controles_omori.csv) est probablement ouvert.\n"
...
with col_title:
    st.title("Contrôle de poids 🧪 – Ligne OMORI 1")
    st.markdown(
...
if poids_produit > 0:
    ...
    if poids_min is not None:
        ...
        st.error(
            ...
            "Ajoute-le dans le dictionnaire `POIDS_MIN_OMORI1` au début du fichier."
        )
else:
